...
def base2to10(binary)
	# num = 0
	# new_arr = binary.split("").map { |e| e.to_i  }
	# base = 1
	# new_arr.reverse.each do |el|
	# 	num += el*base
	# 	base *=2
	# end
	# num
	return binary if binary.length <= 1
	rem = binary.to_i
	num = ((rem%10)*1 )
	result = base2to10((rem/10).to_s).to_i
	num += result*2
	...
	def my_merge(hash, &prc)
		...
		hash.each do |key, value|
			...
			if dup[key].nil?
				...
				dup[key] = prc.call(key,dup[key],value)
			end
		end
		dup
		# your code goes here
...
def longest_palindrome(string)
	...
	arr.each_with_index do |el, idx|
		...
		while idx2 < string.length
			temp = arr[idx..idx2].join("")
			if temp == temp.reverse && temp.length > largest.length
				largest = temp
			end
			idx2 +=1
		end
