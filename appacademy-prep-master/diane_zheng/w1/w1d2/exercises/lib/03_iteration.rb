  ...
  def bubble_sort! (&prc)
    ...
    while not_sorted
      ...
      while idx1 < arr.length - 1
        if prc.call(arr[idx1], arr[idx1 +1]) == 1
          not_sorted = true
          arr[idx1],arr[idx1 +1] = arr[idx1 +1],arr[idx1]
        end
        idx1 +=1
      end
    end
...
def substrings(string)
  ...
  string.each_char.with_index do |char, index|
    unless substring.include?(char)
      substring << char
    end
    index2 = index + 1
    while index2< string.length
      unless substring.include?(string[index..index2])
...
class Array
  ...
  def my_inject(&blk)
    ...
    arr = self[1..-1]
    arr.my_each do |var|
      final_var = blk.call(final_var, var)
    end
  end
end
