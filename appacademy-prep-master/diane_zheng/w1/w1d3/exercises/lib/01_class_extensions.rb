...
class String
  def caesar(shift)
    ...
    a_ascii = 97
    z_ascii = 122
    self.downcase.each_byte do |ascii_char|
      ascii = ascii_char + shift
      if ascii > 122
        ascii = a_ascii + ascii - z_ascii -1
      end
      arr << ascii.chr
    end
    arr.join("")
...
class Fixnum
  ...
  def stringify(base)
    ...
    while temp_num >0
      value = temp_num%base
      words = digit[value] + words

      temp_num = (temp_num - value)/base
    end
    words
  end
end
