...
class Course
  ...
  def initialize(name, department, credits, days = [:mon], time_block = 1)
    @students =[]
    @name = name
  ...
  def conflicts_with?(crs)
    conflict = false
    if self.time_block == crs.time_block
      self.days.each do |day1|
        crs.days.each do |day2|
          if day1 == day2
            conflict = true
          end
        end
